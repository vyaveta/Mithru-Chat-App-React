 ...
 function SetAvatar() {
    ...
    const setProfilePicture = async () => {
      if(selectedAvatar === undefined){
        toast.error('Please select an avvatar',toastOptions)
      }else{
        const user = await JSON.parse(localStorage.getItem('mithru-app-user'))
        const {data} = await axios.post(`${setAvatarRoute}/${user._id}`,{image:avatars[selectedAvatar]})
        console.log(data,'is the data')
        if(data.isSet){
          user.isAvatarImageSet = true;
          user.avatarImage = data.image;
          localStorage.setItem('mithru-app-user',JSON.stringify(user))
          navigate('/')
        }else{
